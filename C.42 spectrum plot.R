...
# Determine number of datapoints for each discrete wavelength
  no.points<-grep("Number of Points",fixed=TRUE,data[,1])
  points<-as.numeric(array(unlist(data[no.points[1],2])))-1
  largemer.point<-13731+2
  smallmer.point<-14677+2
  largemer.time<-as.numeric(sub(" Min","",array(unlist(data[largemer.point,2]))))
  smallmer.time<-as.numeric(sub(" Min","",array(unlist(data[smallmer.point,2]))))

  largemer.end<-largemer.point+points
  smallmer.end<-smallmer.point+points 
  
  # Gets absorbance data for 24mer and monomer and wavelength
  wavelength<-as.numeric(array(unlist(data[largemer.point:largemer.end,1])))
  largemer.absorbance<-as.numeric(array(unlist(data[largemer.point:largemer.end,2])))
  smallmer.absorbance<-as.numeric(array(unlist(data[smallmer.point:smallmer.end,2])))
...
{
  setwd(figure.dir)
  png(file="Wildtype GLFG Absorption Spectra.png", bg="transparent", width =1000, height=500,units="px",pointsize=13)
  plot(x=data[,1],y=data[,2],type='l',col=colours[1],xlab="Wavelength",ylab="Relative Absorbance (A.U)",main="Wildtype GLFG Absorption Spectra", 
